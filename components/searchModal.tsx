...
import { searchCoins, SearchCoin } from '@/lib/coingecko.actions';
...
const SearchItem = ({ coin, onSelect, isActiveName }: SearchItemProps) => {
  // Determine if this is a SearchCoin (from search) or TrendingCoin['item'] (from trending)
          ...
          {isResultsVisible && (
            ...
            >
              {searchResults.slice(0, SEARCH_LIMIT).map((coin) => (
                <SearchItem
                  key={coin.id}
                  coin={coin as unknown as SearchItemCoin}
                  onSelect={handleSelect}
                  isActiveName
                />
              ))}
